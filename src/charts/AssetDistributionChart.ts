...
export class AssetDistributionChart extends BaseChart {
  ...
  async draw(): Promise<Canvas> {
    ...
    assets.forEach(([symbol, asset], index) => {
      ...
      this.ctx.fillText(
        `${symbol}: ${(percentage * 100).toFixed(1)}%`,
  ...
  private drawLegend(assets: [string, any][], colors: string[]): void {
    const legendX = 50;
    ...
    assets.forEach(([symbol, asset], index) => {
      ...
      this.ctx.fillText(
        `${symbol}: $${asset.tvl.toLocaleString()} (${asset.apr.toFixed(2)}%)`,
        legendX + 30,
        y + 12
      );
    });
