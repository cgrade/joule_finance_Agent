...
export class JouleBlockchainData {
  ...
  private readonly JOULE_CONFIG_ADDRESS = '0x2fe576faa841347a9b1b32c869685deb75a15e3f62dfe37cbd6d52cc403a16f6';
  private readonly JOULE_POOL_MODULE = 'pool';
  private readonly JOULE_LENDING_MODULE = 'lending';
  ...
  async fetchTVL(): Promise<{
    ...
    try {
      ...
      const poolResources = resources.filter(res => 
        res.type.includes(`${this.JOULE_CONFIG_ADDRESS}::${this.JOULE_POOL_MODULE}`)
      );
  ...
  async fetchAPYRates(): Promise<{
    ...
    try {
      ...
      const lendingResources = resources.filter(res => 
        res.type.includes(`${this.JOULE_CONFIG_ADDRESS}::${this.JOULE_LENDING_MODULE}`)
      );
      
      // Extract lending and borrowing rates
      const lendingRates: Record<string, number> = {};
      const borrowingRates: Record<string, number> = {};
   ...
   */
  private extractAssetTypeFromResource(resourceType: string): string {
    ...
    if (matches && matches[1]) {
      ...
      if (assetName === 'AptosCoin') return 'APT';
      if (assetName.includes('USD')) return assetName.toUpperCase();
      return assetName;
    }
    return 'Unknown';
  }
   ...
   */
  private async convertToUSD(assetType: string, amount: number): Promise<number> {
    try {
