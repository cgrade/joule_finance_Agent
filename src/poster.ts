 ...
 */
export const createBullishPostSystem = async (): Promise<(userInput: string) => Promise<any>> => {
  try {
...
export const runAsBullishPoster = async (userInput?: string): Promise<void> => {
  try {
    ...
    const postingResult = result.poster_output?.find((msg: BaseMessage) => msg.name === 'posting_agent');
    if (postingResult) {
