...
export class JouleFinanceDataTool {
    ...
    constructor(agentRuntime, options = {}) {
        ...
        console.log(chalk.cyan(`Initialized Joule Finance data tool with ${isTestnet ? 'TESTNET' : 'MAINNET'} endpoint: ${this.apiEndpoint}`));
        // Log if mock data is allowed
        if (this.allowMockData) {
            console.log(chalk.yellow("⚠️ Mock data is enabled - will use simulated data if blockchain data is unavailable"));
        }
        else {
            console.log(chalk.green("✅ Mock data is disabled - will only return real blockchain data or error state"));
        }
    }
    /**
