            ...
            invoke: async (stateObject) => {
                ...
                try {
                    ...
                    try {
                        ...
                        if (jouleFinanceTool && typeof jouleFinanceTool.getData === 'function') {
                            marketData = await jouleFinanceTool.getData() || marketData;
                        }
                    }
                    catch (error) {
                ...
                ${Object.entries(marketData.assets)
                                        ...
                                        .slice(0, 3)
                                        .map(([asset, data]) => `- ${asset}: Price $${data.price.toFixed(2)}, TVL $${(data.tvl / 1000000).toFixed(2)}M, APR ${data.apr.toFixed(2)}%`).join('\n')}`
...
function getMessageContent(result) {
...
async function createChartFromMarketData(xPostTool, marketData) {
    try {
        // Use type assertion to access methods
        const tool = xPostTool;
        // Try different ways to access the visualization tool
